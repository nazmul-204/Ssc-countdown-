...
2. `vercel.json` in this repo pins the framework explicitly, so Vercel doesn't need to guess:
...
> **If you previously ran `npm install` on your own machine (Mac/Windows) and it created a `package-lock.json`, delete it before your first push**, or commit it fresh right before pushing. A lockfile generated on one OS can pin the wrong platform-specific `esbuild` binary and make the build fail on Vercel's Linux builders right after the `transforming...` line — this is the single most common cause of that exact failure signature. This repo intentionally ships without a lockfile so Vercel generates a correct one on first install.
